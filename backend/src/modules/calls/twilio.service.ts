...
export class TwilioService {
  ...
  constructor(private configService: ConfigService,   private clickhouse: ClickhouseService,) {
  ...
  async makeCall(to: string, userId: string) {
    const from = this.configService.get<string>('TWILIO_PHONE_NUMBER') as string;
